    ...
    for epoch in range(num_epochs):
        ...
        if iou > best_iou:
            ...
            torch.save({
                ...
                'best_iou': best_iou,
                'history': history,
            }, 'best_LandslideUNet_checkpoint.pth')
...
class BoundaryAwareLoss(nn.Module):
    ...
    def forward(self, pred, target, boundary_pred=None, boundary_target=None):
        # 主分割损失
        seg_loss = self.bce_loss(pred, target) + self.dice_loss(pred, target)

        if boundary_pred is not None and boundary_target is not None:
            # 边界损失
            boundary_loss = self.bce_loss(boundary_pred, boundary_target)
            # 总损失
...
def main():
    ...
    criterion = BoundaryAwareLoss(alpha=0.7, beta=0.3)
    ...
    if device_ids and len(device_ids) > 1:
        # 多GPU训练时，保存module
        torch.save(train_model.module.state_dict(), '/kaggle/working/final_LandslideUNet_model.pth')
    else:
        torch.save(train_model.state_dict(), '/kaggle/working/final_LandslideUNet_model.pth')
    print("最终模型已保存为 'final_LandslideUNet_model.pth'")
    print("训练完成!")
    # 1. 加载训练好的模型

    model.load_state_dict(torch.load('/kaggle/working/final_LandslideUNet_model.pth'))
    model.eval()
