...
class U2Net_RS_Final(nn.Module):
    def __init__(self, in_ch=4, out_ch=1):
        ...
        self.pool34 = nn.MaxPool2d(2, stride=2, ceil_mode=True)  # 名字统一为 pool34
    ...
    def forward(self, rgb, dem):
        ...
        hx3 = self.stage3(hx)
        hx = self.pool34(hx3)  # 必须使用 self.pool34
...
def get_simple_training_config():
    ...
    # 1. 基础 Loss (修正版)
    def core_loss_func(pred, target):
        # BCE
    ...
    def deep_supervision_loss(preds, target):
        ...
        loss_total = 0
        # 权重：给 d0 最高权重，其他辅助
        weights = [1.0, 0.8, 0.6, 0.4, 0.2, 0.1, 0.1]
        ...
        for i, pred in enumerate(preds):
            ...
            current_loss = core_loss_func(pred, target_resized)

            if i < len(weights):
                loss_total += weights[i] * current_loss
            else:
    ...
    return model, deep_supervision_loss, optimizer, scheduler
