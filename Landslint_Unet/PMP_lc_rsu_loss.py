...
def get_simple_training_config_enhance():
    ...
    # 1. 创建简单模型
    model = U2Net_RS_Final(in_ch=4, out_ch=1).to('cuda')
...
def get_simple_training_config():
    """
    获取优化后的训练配置
    策略：针对IoU已达0.77但存在漏检的情况，使用 Focal + Tversky Loss 强力提升召回率
    """

    # 1. 创建简单模型 (保持你现在的模型)
    model = U2Net_RS_Final(in_ch=4, out_ch=1).to('cuda')

    # 2. 定义终极损失函数：Focal + Tversky (Recall Boost版)
    def combined_loss_recall_boost(pred, target):
        """
        Focal Loss (挖掘难样本) + Tversky Loss (侧重召回率，解决漏检)
        """
        pred = pred.float()
        target = target.float()

        # --- A. Focal Loss 部分 (专注难分样本) ---
        # 能够让模型关注那些"看着像背景的滑坡"
        bce = F.binary_cross_entropy_with_logits(pred, target, reduction='none')
        pt = torch.exp(-bce)
        # alpha=0.75 意味着我们给正样本(滑坡)更高的基础权重
        # gamma=2.0 保持对难样本的挖掘力度
        focal_loss = (0.75 * (1 - pt) ** 2.0 * bce).mean()

        # --- B. Tversky Loss 部分 (强力惩罚漏检) ---
        # Tversky index = TP / (TP + alpha*FP + beta*FN)
        # 当 beta > alpha 时，模型会更倾向于减少 FN (漏检)，从而把微弱的滑坡"捞"出来
        probs = torch.sigmoid(pred)

        # 这里的 beta=0.7, alpha=0.3 是关键
        # 意思是：漏检的惩罚权重(0.7) 是 误检惩罚权重(0.3) 的两倍多
        alpha_t = 0.3
        beta_t = 0.7
        smooth = 1.0
        ...
        tversky_index = (tp + smooth) / (tp + alpha_t * fp + beta_t * fn + smooth)
        tversky_loss = 1 - tversky_index.mean()

        # --- C. 总损失 ---
        # 0.5 * Focal + 1.0 * Tversky
        return 0.5 * focal_loss + tversky_loss

    # 3. 优化器 (保持 AdamW，稍作微调)
    optimizer = torch.optim.AdamW(
        model.parameters(),
        lr=2e-4,  # IoU很高了，学习率可以稍微给一点点活力，或者保持1e-4
        weight_decay=1e-4
    )
    ...
    # 返回特定的 Recall Boost Loss
    return model, combined_loss_recall_boost, optimizer, scheduler
