...
class RSU7(nn.Module):
    def __init__(self, in_ch=3, mid_ch=12, out_ch=3, use_ca=True):
...
class RSU6(nn.Module):
    def __init__(self, in_ch=3, mid_ch=12, out_ch=3, use_ca=True):
        ...
        self.rebnconv2d = REBNCONV(mid_ch * 2, mid_ch, dirate=1)
        self.rebnconv1d = REBNCONV(mid_ch * 2, out_ch, dirate=1)
        if self.use_ca: self.ca = CoordAtt(out_ch, out_ch)
    ...
    def forward(self, x):
        ...
        hx2dup = F.interpolate(hx2d, size=hx1.shape[2:], mode='bilinear', align_corners=True)
        hx1d = self.rebnconv1d(torch.cat((hx2dup, hx1), 1))
        if self.use_ca: hx1d = self.ca(hx1d)
        return hx1d + hxin
...
class RSU4F_Drop(nn.Module):
    def __init__(self, in_ch=3, mid_ch=12, out_ch=3, drop_prob=0.3, use_ca=True):
...
# 3. 终极一键消融实验主网络
# ==========================================
class U2Net_Ablation(nn.Module):
    def __init__(self, out_ch=1,
                 use_dual=True,  # 是否使用双流输入
                 use_ca=True,  # 是否在RSU中使用CoordAtt
                 use_smooth=True,  # 是否使用Smooth Head
                 use_se=True):  # 是否使用SEFusion
        super(U2Net_Ablation, self).__init__()
        ...
        if self.use_dual:
            ...
            # 不用双流时，直接把 4 通道融合到 64 通道，对齐 Stage1 的输入
            self.early_fusion = nn.Sequential(
                nn.Conv2d(4, 64, 3, padding=1, bias=False),
                nn.BatchNorm2d(64), nn.ReLU(inplace=True))

        # --- 模块2: RSU层 (带 CA 消融开关) ---
        self.stage1 = RSU7(64, 32, 64, use_ca=use_ca)
        self.pool12 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        self.stage2 = RSU6(64, 32, 128, use_ca=use_ca)
        self.pool23 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        self.stage3 = RSU5(128, 64, 256, use_ca=use_ca)
        self.pool34 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        self.stage4 = RSU4(256, 128, 512, use_ca=use_ca)
        self.pool45 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        self.stage5 = RSU4F_Drop(512, 256, 512, drop_prob=0.2, use_ca=use_ca)
        self.pool56 = nn.MaxPool2d(2, stride=2, ceil_mode=True)
        self.stage6 = RSU4F_Drop(512, 256, 512, drop_prob=0.3, use_ca=use_ca)

        self.stage5d = RSU4F(1024, 256, 512, use_ca=use_ca)
        self.stage4d = RSU4(1024, 128, 256, use_ca=use_ca)
        self.stage3d = RSU5(512, 64, 128, use_ca=use_ca)
        self.stage2d = RSU6(256, 32, 64, use_ca=use_ca)
        self.stage1d = RSU7(128, 16, 64, use_ca=use_ca)

        # --- 模块3: 平滑预测头消融 ---
        def get_side_layer(in_c):
            if self.use_smooth:
                return SmoothHead(in_c, out_ch)
            else:
                return nn.Conv2d(in_c, out_ch, 3, padding=1)

        self.side1 = get_side_layer(64)
        self.side2 = get_side_layer(64)
        self.side3 = get_side_layer(128)
        self.side4 = get_side_layer(256)
        self.side5 = get_side_layer(512)
        self.side6 = get_side_layer(512)
    ...
    def forward(self, rgb, dem):
        ...
        if self.use_dual:
            ...
            feat_dem = self.conv_dem(dem)
            x = torch.cat([feat_rgb, feat_dem], dim=1)
        else:
...
def get_simple_training_config():
    ...
    # 加入双流输入(U2Net + Dual)
    model = U2Net_Ablation(use_dual=True, use_ca=True, use_smooth=False, use_se=False)

    # 加入双流 + CoordAtt (U2Net + Dual + CA)
    # model = U2Net_Ablation(use_dual=True, use_ca=True, use_smooth=False, use_se=False)

    # 加入双流 + CoordAtt + 平滑头 (U2Net + Dual + CA + Smooth)
    # model = U2Net_Ablation(use_dual=True, use_ca=True, use_smooth=True, use_se=False)
...
def main():
    ...
    if device_ids and len(device_ids) > 1:
        # 多GPU训练时，保存module
        torch.save(train_model.module.state_dict(), '/kaggle/working/U2Net_Dual.pth')
    else:
        torch.save(train_model.state_dict(), '/kaggle/working/U2Net_Dual.pth')
    print("最终模型已保存为 'U2Net_Dual.pth'")
    print("训练完成!")
    # 1. 加载训练好的模型

    model.load_state_dict(torch.load('/kaggle/working/U2Net_Dual.pth'))
    model.eval()
