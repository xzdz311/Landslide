...
def get_simple_training_config():
    ...
    # 1. 创建简单模型
    model = SegNet(n_channels=3, n_classes=1).to('cuda')
...
def main():
    ...
    if device_ids and len(device_ids) > 1:
        # 多GPU训练时，保存module
        torch.save(train_model.module.state_dict(), '/kaggle/working/final_SegNet_nodem_model.pth')
    else:
        torch.save(train_model.state_dict(), '/kaggle/working/final_SegNet_nodem_model.pth')
    print("最终模型已保存为 'final_SegNet_model.pth'")
    print("训练完成!")
    # 1. 加载训练好的模型

    model.load_state_dict(torch.load('/kaggle/working/final_SegNet_nodem_model.pth'))
    model.eval()
